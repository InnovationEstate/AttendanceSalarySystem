...
export default function getAttendanceSummary(
  ...
  for (let day = 1; day <= totalDays; day++) {
    ...
    // ✅ If Admin explicitly set a status → always use it
    if (record?.status) {
      let normalizedStatus = record.status;
      // Normalize any type of leave
      if (normalizedStatus.toLowerCase().includes("leave")) {
        normalizedStatus = "Leave";
      }

      detailedDays.push({ day, status: normalizedStatus });

      if (normalizedStatus === "Present") present++;
      else if (normalizedStatus === "Absent") absent++;
      else if (normalizedStatus === "Leave") leave++;
      else if (normalizedStatus === "Half Day") half++;
      else if (normalizedStatus === "Week Off") weekOff++;

      continue; // skip login/minutes logic
    }
