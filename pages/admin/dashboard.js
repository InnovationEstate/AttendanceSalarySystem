...
import { db } from "@/lib/firebase"; // Make sure this exports the initialized db
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const empData = empSnap.exists() ? Object.values(empSnap.val()) : [];

        // Fetch Today's Attendance
        const today = getTodayDate();
        const attSnap = await get(child(dbRef, `attendance/${today}`));
        const attData = attSnap.exists() ? Object.entries(attSnap.val()).map(([email, data]) => ({
          email,
          ...data,
          date: today,
        })) : [];
        ...
        calculateSummary(empData, attData);
      } catch (err) {
  ...
  function calculateSummary(empData, attData) {
    ...
    const now = new Date();
    const isTuesday = now.toLocaleString("en-US", { weekday: "long" }) === "Tuesday";
    ...
    employees.forEach((emp) => {
      ...
      switch (type) {
        ...
        case "Present":
          ...
          break;
        case "Half Day":
          if (loginMinutes !== null && loginMinutes > 660 && loginMinutes <= 1120) list.push(emp);
          break;
        case "Absent":
  ...
  const StatCard = ({ title, value, clickable = false }) => (
    ...
    >
      ...
      <div className="text-xs sm:text-sm text-gray-700 mt-1 font-medium">{title}</div>
    </div>
  );
  ...
  return (
    <main className="p-4 sm:p-6 bg-gray-100 min-h-screen">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
        <StatCard title="Total Employees" value={summary.totalEmployees}  clickable/>
