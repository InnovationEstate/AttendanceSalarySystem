...
export default function AdminDocuments() {
    ...
    async function fetchEmployees() {
      ...
      try {
        ...
        empSnap.forEach(childSnap => {
          // childSnap.key may be numeric index or ID; val contains employee data.
  ...
  // Fetch documents of selected employee from Firebase
  const fetchDocuments = async (employeeId) => {
    setSelectedEmp(employeeId);
    setFiles({});
    setMessage("Loading documents...");
    try {
      ...
      if (docsSnap.exists()) {
        const data = docsSnap.val(); // expects object like { docKey: filename }
        setFiles(data);
        setMessage("");
      } else {
  ...
  // View document logic (download blob and open)
  const viewDocument = async (docKey) => {
    if (!selectedEmp) {
  ...
  const downloadDocument = async (docKey) => {
    ...
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = files[docKey] || `${selectedEmp}_${docKey}`;
      document.body.appendChild(a);
      a.click();
      a.remove();
      URL.revokeObjectURL(url);
    ...
    <div className="bg-gray-50 flex justify-center items-start p-6">
      <div className="max-w-6xl w-full grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 bg-white rounded shadow p-6">
          ...
          <div className="space-y-3">
            {!message && selectedEmp && Object.entries(files).length === 0 && (
              <p>No documents uploaded yet.</p>
            )}
            {Object.entries(files).map(([docKey, filename]) => (
              <div
