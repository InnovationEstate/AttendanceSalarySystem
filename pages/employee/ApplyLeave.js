...
export default function ApplyLeave() {
  ...
  const fetchLeaveHistory = async (email) => {
    ...
    setLeaveList(filtered.sort((a, b) => new Date(b.date) - new Date(a.date)));
  };
  ...
  const handleApply = async () => {
    ...
    const res = await fetch("/api/leave/apply", {
      method: "POST",
      body: JSON.stringify({ email: employee.email, date, reason }),
    });
    ...
    setDate("");
    setReason("");
    fetchLeaveHistory(employee.email); 
  };
  ...
  <div className="max-w-xl mx-auto px-4 sm:px-6 py-6">
    ...
    <div className="bg-white shadow-md rounded-xl border border-gray-200 p-4 sm:p-5 mb-8">
      <div className="space-y-4">
        ...
        {message && (
          <p className="text-sm text-green-600 text-center font-medium mt-2">{message}</p>
        )}
      </div>
    </div>
          ...
          >
            ...
            {leave.status === "approved" && (
              <p className="text-green-700 text-xs mt-1">
                ✅ Leave approved and marked in your attendance.
              </p>
            )}
          </div>
        ))}
