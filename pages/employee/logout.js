...
export default function LogoutPage() {
  ...
  // Logout function clears all storage and redirects
  const doLogout = () => {
    // Clear everything
    localStorage.clear();
  ...
  useEffect(() => {
    // If no employee/role found in storage, redirect immediately
    const employee = localStorage.getItem("employee");
    const role = localStorage.getItem("role");
    const isLoggedIn = sessionStorage.getItem("isEmployeeLoggedIn");

    if (!employee || !role || !isLoggedIn) {
      doLogout();
    }
  }, []);
